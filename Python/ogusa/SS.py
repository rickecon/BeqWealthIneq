...
def euler_equation_solver(guesses, params):
    ...
    r, w, T_H, factor, j, J, S, BQ_dist, beta, sigma, ltilde, g_y,\
    ...
    foc_save_parms = (omega_SS, e[:, j], sigma, BQ_dist[:, j], beta, g_y, chi_b[j], theta, tau_bq[j], rho, lambdas[j], J, S,
    ...
    foc_labor_params = (omega_SS, e[:, j], BQ_dist[:, j], sigma, g_y, theta, b_ellipse, upsilon, chi_n, ltilde, tau_bq[j], lambdas[j], J, S,
    ...
    tax1_params = (e[:, j], BQ_dist[:, j], lambdas[j], 'SS', retire, etr_params, h_wealth, p_wealth, 
                   m_wealth, tau_payroll, theta, tau_bq[j], J, S)
    tax1 = tax.total_taxes(r, w, b_s, n_guess, BQ, factor, T_H, None, False, tax1_params)
    cons_params = (e[:, j], BQ_dist[:, j], lambdas[j], g_y)
...
def inner_loop(outer_loop_vars, params):
    ...
    for j in xrange(J):
        ...
        euler_params = [r, w, T_H, factor, j, J, S, BQ_dist, beta, sigma, ltilde, g_y,\
                  g_n_ss, tau_payroll, retire, mean_income_data,\
                  h_wealth, p_wealth, m_wealth, b_ellipse, upsilon,\
                  j, chi_b, chi_n, tau_bq, rho, lambdas, omega_SS, e,\
                  analytical_mtrs, etr_params, mtrx_params,\
